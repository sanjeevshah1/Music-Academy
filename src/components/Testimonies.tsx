'use client'
import { InfiniteMovingCards } from "./ui/infinite-moving-cards";
import { TypewriterEffectSmooth } from "./ui/typewriter-effect";
const words = [
  {
    text: "Hear",
    className: "text-5xl  font-bold relative z-20 bg-clip-text text-transparent bg-gradient-to-b from-purple-200 to-purple-900 py-8"
  },
  {
    text: "Our",
    className: "text-5xl  font-bold relative z-20 bg-clip-text text-transparent bg-gradient-to-b from-purple-200 to-purple-900 py-8"
  },
  {
    text: "Harmony",
    className: "text-5xl  font-bold relative z-20 bg-clip-text text-transparent bg-gradient-to-b from-purple-200 to-purple-900 py-8"
  },
  {
    text: ":",
    className: "text-5xl  font-bold relative z-20 bg-clip-text text-transparent bg-gradient-to-b from-purple-200 to-purple-900 py-8"
  },
  {
    text: "Voices",
    className: "text-blue-500 dark:text-blue-500",
  },
  {
    text: "Of",
    className: "text-blue-500 dark:text-blue-500",
  },
  {
    text: "Success",
    className: "text-blue-500 dark:text-blue-500",
  },
];
const SchoolTestimonials = [
    {
      quote:
        'Joining the music school transformed my understanding of music and helped me to truly discover my own sound. The instructors are world-class!',
      name: 'Alex Johnson',
      title: 'Guitar Student',
    },
    {
      quote:
        "The community and support at this school are unmatched. I've grown not just as a pianist, but also as a performer, thanks to their comprehensive approach.",
      name: 'Samantha Lee',
      title: 'Piano Student',
    },
    {
      quote:
        "This school offered me the tools and confidence to take my singing to the next level. I'm endlessly grateful for the personalized coaching.",
      name: 'Michael Chen',
      title: 'Vocal Student',
    },
    {
      quote:
        'As a violinist, finding the right mentor can be challenging, but this school matched me with a teacher who truly understands my goals and challenges.',
      name: 'Emily Taylor',
      title: 'Violin Student',
    },
    {
      quote:
        'The production courses here opened my eyes to the intricacies of music production. Highly recommend for any aspiring producers!',
      name: 'Chris Morales',
      title: 'Music Production Student',
    },
  ];

  export default function MusicSchoolTestimonials() {
  return (
    <div className="h-[40rem] w-full bg-black bg-grid-white/[0.2] relative flex flex-col items-center justify-center overflow-hidden">
        <TypewriterEffectSmooth words={words} />

        <div className="mt-8 flex justify-center w-full overflow-hidden px-4 sm:px-6 lg:px-8">
            <div className="w-full max-w-6xl">
            <InfiniteMovingCards
                items={SchoolTestimonials}
                direction="left"
                speed="slow"
      />
            </div>
        </div>
    </div>
  )
}

